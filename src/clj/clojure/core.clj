...
(defn reduced?
  ...
  {:tag java.lang.Boolean
   ...
   :inline-arities #{1}
   :added "1.5"}
  (^boolean [x] (clojure.lang.RT/isReduced x)))
                       ...
                       (let [gvec (gensym "vec__")]
                         (loop [ret (-> bvec (conj gvec) (conj val))
                           ...
                           (if (seq bs)
                             (let [firstb (first bs)]
                               (cond
                                (or (= firstb 'clojure.core/&)
                                    (= firstb '&))
                                (recur (pb ret (second bs) (list `nthnext gvec n))
                                       n
                                       (nnext bs)
                                       true)
...
(defn mix-collection-hash
  ...
  {:added "1.6"
   :static true}
  ^int
  [^int hash-basis ^int count] (clojure.lang.Murmur3/mixCollHash hash-basis count))
...
(defn isa?
  ...
  {:added "1.0"
   :tag Boolean}
  (^boolean [child parent] (isa? global-hierarchy child parent))
  (^boolean [h child parent]
     (boolean (or (= child parent)
...
(defn qualified-specials?
  []
  (. clojure.lang.Compiler (isQualifiedSpecials)))
